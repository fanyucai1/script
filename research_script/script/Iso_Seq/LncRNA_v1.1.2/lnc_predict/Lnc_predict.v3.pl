...
################################
my $_submit_sge_="/lustre/xugl/Programs/Submit/submit_sge.pl";
...
=cut
my $CPAT_BIN = "$Bin/cpat.pl";
my $CPC_BIN     = "/share/nas2/genome/biosoft/cpc/bin/run_predict_local_v2.sh";
...
if ($step==0){
	...
	my $CPAT_dir="$od/CPAT";
	print OUT0 "rm -rf $CPAT_dir && perl $CPAT_BIN -fa $fa -od $CPAT_dir \n";
	close OUT0;
...
if ($step == 1) {
	...
	my $CPC_dir = "$od/CPC";
	print OUT1 "rm -rf $CPC_dir && mkdir $CPC_dir && cd $CPC_dir && ";
...
if ($step == 2) {
	...
	my $CNCI_dir = "$od/CNCI";
	print OUT2 "rm -rf $CNCI_dir && mkdir $CNCI_dir && cd $CNCI_dir  && ";
	print OUT2 " $PYTHON $CNCI_BIN  -f  $fa  -o $CNCI_dir -p 12 -m $Type  \n";
	close OUT2;
	$step++ unless ($oneStepOnly) ;
}
my %FA1;
if ($step == 3) {
	...
	close OUT3;
#	$step++ unless ($oneStepOnly) ;
	&Cut_shell_qsub("$od/work_sh/Predict.sh",50, "30G",  "general.q",1);
...
sub Cut_shell_qsub {
	# &Cut_shell_qsub($shell,$cpu,$vf,$       );
	my $shell = shift;
	my $cpu = shift;
	my $vf = shift;
	my $queue = shift;
	my $nline = shift;
	`perl $_submit_sge_ --queue $queue -maxproc $cpu --independent --lines $nline $shell --resource vf=$vf --reqsub`;
