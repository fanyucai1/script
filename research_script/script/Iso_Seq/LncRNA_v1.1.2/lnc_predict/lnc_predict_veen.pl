...
if ($cpc) {
	...
	while (<CPC>) {
		...
		next if (/\#/||/^$/) ;
		my $line = $_;
		my ($trans_id,$type) = (split /\s+/,$_)[0,2];
...
if($cpat){
	...
	while(<IN>){
		...
		next if (/^#/||/^\s*$/);
		my $line=$_;
		my ($Lnc_id,$label)=(split /\s+/,$line)[1,7];
		#$Lnc_id=(split ":",$Lnc_id)[0];
		$Lnc_id=~s/(^\s+|\s+$)//g;
		#print "#cpatid $Lnc_id ::::$Lnc_id=$faLabel{$Lnc_id} ::::\n";
		if($label eq 'no'){
			push @{$lnc_ids{'cpat'}},$faLabel{$Lnc_id};
			#print"#cpat2= $faLabel{$Lnc_id} \n";
		}
	}
	close IN;
}
