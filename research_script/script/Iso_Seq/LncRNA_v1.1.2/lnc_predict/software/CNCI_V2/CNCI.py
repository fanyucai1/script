...
Home = os.path.split( os.path.realpath( sys.argv[0] ) )[0] 
print Home
################################################################
if ClassModel == 've':
    MatrixPath = Home+"/CNCI_Parameters/CNCI_matrix"
    inMatrix = open(MatrixPath)
    Matrix = inMatrix.read()
    inMatrix.close()
if ClassModel == 'pl':
    MatrixPath = Home+"/CNCI_Parameters/plant.matrix"
    inMatrix = open(MatrixPath)
    Matrix = inMatrix.read()
    inMatrix.close()
...
def PringResult(result,svmfinal):
    ...
    Tabel = 'Transcript_ID' + '\t' + 'index' + '\t' + 'score' + '\t' + 'start' + '\t' + 'end' + '\t' + 'length' + '\n'
...
if FileType:
    ...
    os.system('perl '+Home+'/gtf2Bed.pl '+inPutFileName+' > '+inGtfFiles+'')
    time.sleep(10)
...
################################################################################################
    if ClassModel == 've':
        os.system(Home+'/libsvm-3.0/svm-scale -r ' +Home +'/CNCI_Parameters/python_scale '+SvmoutPutFile+' > '+SvmPutFileName+'')
        os.system(Home+'/libsvm-3.0/svm-predict '+SvmPutFileName+' '+Home+'/CNCI_Parameters/python_model '+SvmFile+' > '+svm_tmp+'')
    if ClassModel == 'pl':
        os.system(Home+'/libsvm-3.0/svm-scale -r '+ Home+'/CNCI_Parameters/plant_scale '+SvmoutPutFile+' > '+SvmPutFileName+'')
        os.system(Home+'/libsvm-3.0/svm-predict '+SvmPutFileName+' '+ Home+'/CNCI_Parameters/plant_model '+SvmFile+' > '+svm_tmp+'')
##########################################################################################################
