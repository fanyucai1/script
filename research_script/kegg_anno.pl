...
my $fasta="/home/fanyucai/software/FASTA_Splitter/fasta-splitter.pl";
...
system "mkdir -p $outdir/split_fa";
...
open(SP,">$outdir/shell/split_fa.sh");
if($type=~/fasta/i)
{
    print SP "perl $fasta --n-parts 10 --out-dir $outdir/split_fa $input\n";
}
system "perl $qsub $outdir/shell/split_fa.sh";
my @seq=glob("$outdir/split_fa/*");
my $k=0;
foreach my $key(@seq)
{
    $k++;
    print KOBAS "$python $kobas_script/annotate.py -i $key -t $type -s $species -n 20 -o $outdir/kobas.$k.out\n";
}
