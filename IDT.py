...
len=args.read-5
##########fastq2bam#####################
cmd="%s -Xmx50g -jar %s FastqToSam F1=%s F2=%s O=%s.unmapped.bam  SM=%s" %(java,picard,args.pe1,args.pe2,out,args.prefix)
...
subprocess.check_call(cmd,shell=True)
##########align reads######################
cmd="%s -Xmx50g -jar %s SamToFastq I=%s.umi.unmapped.bam F=%s.1.fq F2=%s.2.fq"%(java,picard,out,out,out)
subprocess.check_call(cmd,shell=True)
cmd="%s mem -t 10 %s %s.1.fq %s.2.fq >%s.mapped.bam"%(bwa,ref,out,out,out)
subprocess.check_call(cmd,shell=True)
...
cmd="%s -Djava.io.tmpdir=/data/tmp -Xmx50g -jar %s FilterConsensusReads -i %s.consensus.unmapped.bam -o %s.consensus.filter.unmapped.bam -r %s -M 6 3 3 " \
    "-N 50 -E 0.05 -e 0.1 -n 0.05"%(java,fgbio,out,out,ref)
subprocess.check_call(cmd,shell=True)
##############Produce variant calls from consensus reads#################################
cmd="%s -Xmx50g -jar %s SamToFastq I=%s.consensus.filter.unmapped.bam F=%s.consensus.1.fq F2=%s.consensus.2.fq"%(java,picard,out,out,out)
subprocess.check_call(cmd,shell=True)
cmd="%s mem -t 10 %s %s.consensus.1.fq %s.consensus.2.fq >%s.realign.bam"%(bwa,ref,out,out,out)
subprocess.check_call(cmd,shell=True)
cmd="%s -Xmx50g -jar %s MergeBamAlignment UNMAPPED=%s.consensus.filter.unmapped.bam ALIGNED=%s.realign.bam O=%s.consensus.mapped.bam R=%s SO=coordinate MAX_GAPS=-1 ORIENTATIONS=FR VALIDATION_STRINGENCY=SILENT " \
    "ALIGNER_PROPER_PAIR_FLAGS=true CREATE_INDEX=true" %(java,picard,out,out,out,ref)
subprocess.check_call(cmd,shell=True)
cmd="%s -Djava.io.tmpdir=/data/tmp -Xmx50g -jar %s ClipBam -i %s.consensus.mapped.bam -o %s.consensus.mapped.clipped.bam -r %s -s false --clip-overlapping-reads=true"\
