...
def tumor_only(p1,p2,sampelID,outdir):
    ...
    outfile.write("[general]\n"
                  ...
                  sampelID, pe1, pe2))
    outfile.close()
def tumor_normal(m1,m2,tumor,m3,m4,normal,outdir):
    t1=os.path.basename(m1)
    t2 = os.path.basename(m2)
    n1 = os.path.basename(m3)
    n2 = os.path.basename(m4)
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    if not os.path.exists("%s/%s" %(outdir,t1)):
        shutil.copy(m1,outdir)
        shutil.copy(m2,outdir)
        shutil.copy(m3,outdir)
        shutil.copy(m4,outdir)
    outfile = open("%s/run_sm_counter_v2.params.txt" % (outdir), "w")
    outfile.write("[general]\n"
                  "cutadaptDir = /opt/conda/bin/\n"
                  "bwaDir      = /opt/conda/bin/\n"
                  ...
                  "sampleType = normal\n"
                  "duplex = False\n"
                  "refUmiFiles = /project/%s.sum.primer.umis.txt" % (
                  tumor, t1, t2,normal,n1,n2,normal))
    outfile.close()
