...
for key in sample_ID:
    ...
    if os.path.exists(path):
        infile=open(path,"r")
        outfile=open("%s/%s.snv.tmp.vcf"%(args.outdir,key),"w")
        ...
        for line in infile:
            ...
            if num==1:
                ...
                for i in range(len(name)):
                    if name[i]=="VAF" and float(array[i])>=args.vaf:
                        result+=1
        ...
        outfile.close()
        ############################anno snpeff
        cmd="%s -Xmx40g -jar %s/snpEff.jar -v hg19 -canon -hgvs %s/%s.snv.tmp.vcf >%s/%s.snpeff.vcf" %(java,snpeff,args.outdir,key,args.outdir,key)
        subprocess.check_call(cmd,shell=True)
        ############################get knownCanonical transcript
        nm = {}
        file1 = open("%s/%s.snpeff.vcf" % (args.outdir,key), "r")
        for line in file1:
